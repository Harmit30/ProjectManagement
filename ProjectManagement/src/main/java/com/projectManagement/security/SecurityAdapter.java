	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/").authenticated().
			...
			antMatchers("/allProjectForLeader/**","/editInProjectForLeader/**","/projectUpdateEmployeesForLeader/**"
				,"/listOfEmployeeForLeader/**"	
				,"/createTask/**","/TaskDetail/**","/editEmployee/**","/taskList/**").hasAnyRole("LEADER").
			antMatchers("/deleteEmployeeFromProject/**","/addEmployeeToProject/**").hasAnyRole("ADMIN","LEADER").
			antMatchers("/listOfEmployeeForUser/**","/tasklistOfEmployee/**","/changeTaskStauts/**","/editTaskStatus/**").hasRole("USER").
				...
				.logoutRequestMatcher(new AntPathRequestMatcher("/logouts")).logoutSuccessUrl("/logOut").and().csrf().ignoringAntMatchers("/addNewEmployee/**","/updateEmployeeList/**","/deleteEmployeeFromProject/**","/addEmployeeToProject/**");
